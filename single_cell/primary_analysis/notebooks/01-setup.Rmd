...
path_airrflow_objs <- file.path(path_objs, "airr", paste0("airrflow_", airrflow_version))
path_lineages <- file.path(path_airrflow, paste0("bcr_", airrflow_version),
                           "results", "clonal_analysis", "dowser_lineages")

# list (and install if needed) CRAN packages
packages <- c("alakazam", "dowser", "scoper", "shazam", # Immcantation
              "clustree", "DESeq2", "DoubletFinder", "DT", "enrichR",  "ggpubr",
              "ggrepel", "ggsignif", "ggtext", "ggVennDiagram", "here", "hues",
              "janitor", "jsonlite", "kableExtra", "knitr", "magrittr",
              "Matrix", "pals", "patchwork", "peakRAM", "plyr",
              "RColorBrewer", "reshape", "rlang", "scales", "scCATCH", # "Seurat",
              "tidyverse")
# "ggsankey", "networkD3", "pheatmap", "remotes", "rjson", "scCustomize"
# "airr", "SeuratDisk", "lubridate"
# "limma", "edgeR"
...
# load v4 of Seurat, not v5 (and v4.1.4 of SeuratObject too)
library(Seurat, lib.loc = "~/R/seurat_v4")

# list (and install if needed) Bioconductor packages
# have to have BiocManager already installed
packages_additional <- c("biomaRt", "celldex", "ComplexHeatmap", "edgeR",
                         "ggtree", "limma", "Nebulosa", "scDblFinder",
                         "scRNAseq", "scuttle", "SingleCellExperiment",
                         "SingleR")
# BiocParallel
...
b_cell_types <- c("B Cells", "Plasma Cells")
t_cell_types <- "T Cells"

# for plot subtitles
b_cell_types_str <- str_c(b_cell_types, collapse = ", ")
...
colors_datatype <- c("Bulk" = "#788896",
                     "BCR" = "#00A785", "GEX" = "#0CDD6B", "TCR" = "#007860") # shades of green
colors_airrflow <- "#00e5b7"
# colors_isotype <- c("IgA" = "#8538FA", "IgD" = "#F82c7A", "IgE" = "#6718DC",
#                     "IgG" = "#3A0688", "IgM" = "#B50045") # reds and purples
colors_isotype <- c("IgA" = IG_COLORS[["IGHA"]], # blue
                    "IgD" = IG_COLORS[["IGHD"]], # orange
                    "IgE" = IG_COLORS[["IGHE"]], # red
                    "IgG" = IG_COLORS[["IGHG"]], # green
                    "IgM" = IG_COLORS[["IGHM"]]) # purple

# based on IG_COLORS from alakazam (+ base colors blended with black)
# doesn't have everything (e.g. there are 7 IGLCs)
colors_isotype_full <- c("IGHA" = IG_COLORS[["IGHA"]],
                         "IGHA1" = "#25547B",
                         "IGHA2" = "#122A3D",
                         "IGHD" = IG_COLORS[["IGHD"]],
                         "IGHE" = IG_COLORS[["IGHE"]],
...
colors_locus <- c("IGH" = "#2AB0BD", "IGK" = "#DEA618", "IGL" = "#916800",
                  "TRB" = "#085b63", "TRA" = "#57410a") # shades of blue, yellow, green

colors_v_call_igh_fam <- setNames(c("#7690c7", "#8d4bca", "#72b648", "#d16099",
                                    "#57865f", "#62396e", "#bc9149"),
                                  nm = str_c("IGHV", 1:7))
colors_v_call_igk_fam <- setNames(ocean.solar(n = 7),
                                  nm = str_c("IGKV", 1:7))
colors_v_call_igl_fam <- setNames(ocean.haline(n = 10),
                                  nm = str_c("IGLV", 1:10))
colors_v_call_fam <- c(colors_v_call_igh_fam, colors_v_call_igk_fam,
                       colors_v_call_igl_fam)
colors_d_call_fam <- setNames(ocean.turbid(n = 7),
                              nm = str_c("IGHD", 1:7)) # heavy only
colors_j_call_igh_fam <- setNames(ocean.ice(n = 7),
                                  nm = str_c("IGHJ", 1:7))
colors_j_call_igk_fam <- setNames(ocean.algae(n = 7),
                                  nm = str_c("IGKJ", 1:7))
colors_j_call_igl_fam <- setNames(ocean.matter(n = 10),
                                  nm = str_c("IGLJ", 1:10))
colors_j_call_fam <- c(colors_j_call_igh_fam, colors_j_call_igk_fam,
                       colors_j_call_igl_fam)

# for data type comparisons
colors_bcr_gex <- c("GEX" = colors_datatype[["GEX"]], "BCR_GEX" = "black",
                    "BCR" = colors_datatype[["BCR"]])
colors_gex_tcr <- c("GEX" = colors_datatype[["GEX"]], "GEX_TCR" = "black",
                    "TCR" = colors_datatype[["TCR"]])
colors_bcr_tcr <- c("BCR" = colors_datatype[["BCR"]], "BCR_TCR" = "black",
                    "TCR" = colors_datatype[["TCR"]])
...
# QC specific
colors_chain_bcr <- c("HeavyChainOnly" = "#1A759f", "Paired" = "#52b69A",
                      "LightChainOnly" = "#99D98C") # green and blue
...
# for clonal explorations
colors_overlaps <- c("Blood_EM_Only" = colors_sample_types[["Blood"]],
                     "Control" = colors_sample_types[["Control"]],
                     "EM_Only" = colors_sample_types[["EM"]])
colors_expanded <- c("Singlet" = "#BAC893",
                     "Expanded (>1 & <10)" = "#606c38",
                     "Hyperexpanded (10+)" = "#30361C")
colors_expanded_simple <- c("Singlet" = "#003452", "Expanded" = "#00a4ff")
```
