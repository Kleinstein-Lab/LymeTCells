...
```{r}
include_graphics(path = here::here("primary_analysis", "images",
...
The IGs and TRs have *not* been filtered out of the GEX data.
...
print_kable(read_csv(file.path(path_base, "primary_analysis",
                               "cellranger", "10x_Genomics_count.csv"),
                     show_col_types = FALSE))
```

<br>
Directly from [10x Genomics (v6.1) [Immune Profiling]](https://support.10xgenomics.com/single-cell-vdj/software/pipelines/latest/output/metrics):
...
plot_counts(summary_df = metrics_summary, count_type = "Read",
            ...
            aggregation_state = "Unaggregated") %>%
  add_info_bar(info_type = "Dataset") +
  labs(caption = "Solid line: YCGA's recommended amount | dashed line: current minimum") +
...
p <- ggplot(dplyr::filter(metrics_summary, DataType %in% "GEX"),
...
add_info_bar(p, info_type = "Dataset") 
```
...
add_info_bar(p, info_type = "SampleType", label = FALSE) 
```
...
# aggr_results <- rjson::fromJSON(file.path(path_data_root, "aggr", "outs", "count", "summary.json"))

str(aggr_results)
# as.data.frame(aggr_results)
```
...
# preaggregation
add_info_bar(ggplot(dplyr::filter(metrics_summary, DataType == "GEX"),
                    aes(x = SampleName, y = NumberofReads,
                        fill = Dataset)) +
...
# post aggregation
add_info_bar(ggplot(aggr_results_summary,
...
add_info_bar(ggplot(aggr_results_summary,
                    aes(x = SampleName, y = num_cells_by_library,
                        fill = Dataset)) +
               geom_col(color = "black", linewidth = 0.2) +
...
ggVennDiagram(x = list(GEX = unlist(barcodes_GEX),
                       TCR = unlist(barcodes_TCR)),
...
names(L) <- c("GEX_GEX_matrix", "TCR_TCR_matrix",
              "GEX_GEX_matrix_p", "TCR_TCR_matrix_p",
              "GEX_TCR_matrix", "GEX_TCR_matrix_p")
