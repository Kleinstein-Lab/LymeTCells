...
```{r func-plot-bool}
#' Plot boolean selection on PCA or UMAP projections.
#'
#' @description
#' Creates a scatter plot showing a subset of cells highlighted on either PCA or UMAP projections.
#' The function overlays selected cells on top of all plotted cells with different visual properties.
#'
#' @details
#' This function is useful for visualizing specific cell subsets or conditions on dimensionality
#' reduction plots. Selected cells are highlighted with higher alpha values while maintaining
#' the overall context of the full dataset.
#'
#' @param object A Seurat object containing the dimensionality reduction data.
#' @param plot_cell Logical vector indicating which cells to plot. If NULL, all cells are plotted.
#' @param select_cell Character vector of cell names to highlight.
#' @param type Type of dimensionality reduction to plot, either "pca" or "umap".
#' @param COLORS Color palette for cell subsets.
#' @param ASSIGN_DICT Dictionary mapping identities to subset assignments.
#'
#' @returns A ggplot object showing the dimensionality reduction plot with highlighted cells.
...
#' @param col_samples The column name containing sample names. Default is "SampleName".
#' @param col_barcodes The column name containing cell barcodes. Default is "cell_id".
#' @param col_output The name for the output column. Default is "Cell_ID_Unique".
#'
...
#'
#' @description
#' Creates a formatted title string for DotPlots by combining a data description
#' with marker sources in parentheses.
#'
...
#' Create sharing matrix for single-cell data between samples.
#'
#' @description
#' This function creates overlap matrices showing shared cells between samples based
#' on matching barcodes and sequences in single-cell data.
#'
#' @details
#' The function compares cells between samples and identifies those with identical
#' barcodes and sequences. It returns count, total, and percentage overlap matrices.
#'
#' @param clip_df A data frame containing single-cell sequence data.
#' @param group Column name for grouping samples.
#' @param cell Column name for cell barcodes. Default is "CELL".
#' @param seq Column name for sequences. Default is "SEQUENCE_IMGT".
#' @param order_by Optional vector to specify the order of groups in the output matrices.
#'
#' @returns A list containing overlap count, total, and percentage matrices.
...
#' Remove cells with same barcode and sequences between samples.
#'
#' @description
#' Identifies and removes duplicate cells that have identical barcodes and sequences
#' across different sample groups.
#'
#' @details
#' This function finds cells that appear in multiple samples with the same barcode
#' and sequence, marking them for removal to eliminate cross-sample contamination.
#'
#' @param clip_df A data frame containing single-cell sequence data.
#' @param group Column name for grouping samples.
#' @param cell Column name for cell barcodes. Default is "CELL".
#' @param seq Column name for sequences. Default is "SEQUENCE_IMGT".
#'
#' @returns A data frame with duplicate cells removed.
...
#' Remove all duplicates iteratively from single cell data.
#'
#' @description
#' Main function to remove duplicates in single cell data by repeatedly calling
#' removeSingleCellDuplicate until no more duplicates are found.
#'
#' @details
#' This function may need multiple cycles to remove all duplicates, as removing
#' some duplicates may reveal additional ones.
#'
#' @param clip_df A data frame containing single-cell sequence data.
#' @param group Column name for grouping samples.
#' @param cell Column name for cell barcodes. Default is "CELL".
#' @param seq Column name for sequences. Default is "SEQUENCE_IMGT".
#'
#' @returns A data frame with all duplicate cells removed.
...
#' Create an Overlap S4 object.
#'
#' @description
#' Creates an S4 object of class "Overlap" containing matrices for overlap counts,
#' totals, and percentages between samples.
#'
#' @param overlap_count Matrix containing overlap counts between samples.
#' @param overlap_total Matrix containing total counts for each comparison.
#' @param overlap_percent Matrix containing overlap percentages.
#'
#' @returns An Overlap S4 object containing the three matrices.
...
#' Plot overlap heatmap for single-cell data.
#'
#' @description
#' Creates a heatmap visualization showing overlaps between samples in single-cell data,
#' with customizable region of interest and text sizes.
#'
#' @details
#' This function visualizes overlap matrices as heatmaps with counts on diagonals
#' and percentages in off-diagonal elements.
#'
#' @param list_objOverlap List of Overlap objects to plot.
#' @param titleText Title for the plot.
#' @param subtitleText Subtitle for the plot.
#' @param ROI Region of interest to subset the data.
#' @param order_by Vector specifying the order of samples.
#' @param sizeDiag Size of diagonal text elements.
#' @param sizeCount Size of count text elements.
#' @param sizePercent Size of percentage text elements.
#'
#' @returns A ggplot heatmap showing sample overlaps.
plotOverlapSingleCell <- function(list_objOverlap = "",
  ...
  for (x in seq_along(list_objOverlap)) {
    ...
    copa <- c("white",
              colorRampPalette(pals::brewer.oranges(n = 9))(nlevels(data_m$Y1) - 1))
...
#' @param plot_by all samples, by dataset, by sample
#' @param ncol_sample
#'
#' @returns
...
#'
#' @description
#' Creates binned categories for mutation frequency values in a Seurat object
#' to facilitate visualization and analysis.
#'
#' @details
#' The function creates categorical bins from continuous mutation frequency values.
#' When more_bins is TRUE, it uses a more granular binning scheme.
#'
#' @param seurat_obj The Seurat object.
#' @param more_bins Use more bins.
...
#'
#' @description
#' Performs pathway enrichment analysis for differentially expressed genes
#' and creates enrichment plots for each cell type.
#'
#' @details
...
#'
#' @returns A list of enrichment plots for each cell type.
...
#'
#' @description
#' Attempts to read a trees object from an RDS file in the specified directory.
#' Returns NULL if the file doesn't exist.
#'
#' @param dir The directory that contains the tree info.
#'
#' @returns A trees object if the file exists, NULL otherwise.
