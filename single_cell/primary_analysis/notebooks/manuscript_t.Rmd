...
```{r setup-airr}
combined_tcr <-
  readRDS(file.path(path_objs_airrflow, "combined_tcr.rds"))
# combined_bulk_tcr.rds
```
...
#           file.path(path_tables, "manuscript", "clinical.csv"))
```
...
```{r}
metrics_summary_tcr <-
  create_metrics_summary(meta = meta, path_data = path_data_root,
...
#           file.path(path_tables, "manuscript", "counts.csv"))
```
...
# save for SDV spreadsheet
sdv$Supplemental_Figure_11 <- metrics_summary
...
for (df in names(sdv)) {
  write.xlsx(x = as.data.frame(sdv[[df]]),
             file = file.path(path_tables, "manuscript",
