...
I ran `airrflow` with the following script. The clonal threshold was set to `0` since TCRs don't hypermutate.

```{r script-tcr}
# from /media/edelaron/Midas/lyme/airrflow/tcr
readLines(file.path(path_airrflow, paste0("tcr_", airrflow_version),
                    "airrflow.sh")) %>%
  paste0(collapse = "\n") %>% cat
```
...
combined_tcr <-
  plyr::ldply(list.files(file.path(path_airrflow,
...
Redo the clonal assignment:

```{r}
#| eval = FALSE

# "To identify TCR clones, clones were assigned if T cells shared the same full-length aligned V(D)J sequence." - Roy's paper

# using TRB only is okay
combined_tcr_full_clones <-
  combined_tcr_full %>% select(-clone_id, -clone_id_unique)
combined_tcr_full_clones <-
  scoper::identicalClones(test, method = "nt", fields = "Subject",
                          only_heavy = TRUE, split_light = FALSE, nproc = 8,
...
saveRDS(combined_tcr_full,
        file.path(path_objs, "airr", "combined_airrflow_bulk_tcr.rds"))
```
