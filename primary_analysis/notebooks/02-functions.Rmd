...
*Note that you might have to update some functions depending on your version of Seurat e.g. `slot` will have to change to `layer`*
...
```{r func-table}
#' Enhanced table function that shows NAs by default.
#'
#' @description
#' A wrapper around base::table() that includes NA values in the output by default.
#' This is useful for data exploration where you want to see missing values.
#'
#' @param ... Arguments passed to base::table().
#' @param useNA A character string indicating whether to include NA values.
#' Default is "ifany" to show NAs if they exist.
#'
#' @returns A contingency table including NA values when present.
...
#' @param inverted Whether to invert the plot orientation.
...
#' @param info_to_add The options are: `cluster_size`, `percent_TCR`, and `TRUST4`.
#'
...
#' @param specific_clusters Can overlay clusters of interest e.g. T cell or by #.
#' @param specific_col A column from the Seurat object's metadata.
...
create_seurat_objs <- function(objs_list, meta_file,
  ...
  for (sample in sample_names) {
    ...
    if (rlang::is_missing(different_location)) {
    ...
    obj$Subject <- meta_sample$Subject # de-identified ID
...
#' 
#' @details
#' Note that if you are looking at read pairs instead of reads, you should multiply by two.
#'
#' @param meta The metadata file.
#' @param path_data Where the data is located.
#' @param data_types At least one of: GEX, TCR
#' @param samples_list The samples you would like to examine.
#'
#' @returns A data.frame.
create_metrics_summary <- function(meta, path_data, data_types, samples_list) {
  metrics_summary <- tibble()
  if (rlang::is_missing(data_types)) data_types <- unique(meta$DataType)
  ...
  for (sample in samples_list) {
    for (data_type in data_types) {
      # metadata info
      meta_sample <-
        dplyr::filter(meta, SampleName == sample, DataType == data_type)
      ...
      path_data_specific <-
        file.path(path_data, paste0("dataset", dataset), "default")
      data <- read_csv(file.path(path_data_specific, meta_sample$SampleDir,
      ...
      reads_per_cell <- num_reads %/% estimated_number_of_cells  # mean read pairs per cell is also a column
...
#'
#' @description
#' Reformats individual VDJ barcodes by adding a sample name prefix and removing
#' the standard 10X suffix.
#'
#' @details
#' This function processes barcodes from a single sample, splitting on the "-" character
#' and recombining with the sample name to create unique identifiers.
#'
...
#'
#' @returns A character vector of reformatted barcodes with sample prefix.
...
#'
#' @description
#' Reformats cell barcodes by adding sample name prefix and removing suffix numbers,
#' creating unique cell identifiers across multiple samples.
#'
#' @details
#' This function takes a dataset with sample names and cell IDs, separates the barcode
#' portion from any suffix (typically "-1"), and combines it with the sample name
#' to create unique cell identifiers.
#'
#' @param data A data frame containing sample names and cell barcodes.
#' @param col_samples The column name containing sample names.
#' @param col_barcodes The column name containing cell barcodes.
#' @param col_output The name for the output column.
#'
#' @returns A character vector of reformatted unique cell identifiers.
...
#' @param data_types Data types to include, e.g. GEX, TCR, or "All".
#' @param count_type The type of count to plot: "Read", "Cell", "reads_per_cell", or "Barcode".
#' @param aggregation_state Aggregation state, either "Aggregated" or "Unaggregated".
#' @param samples Description of samples being plotted.
#' @param fill_type_label The legend label for fill, e.g. "Data Type" or "Chain Type".
#' @param x_axis What metadata column to plot along the x axis.
#' @param x_axis_label The label for the x axis.
#' @param clrs_datatype Named color vector for the fill types (must be given in correct order if plotting barcodes).
#'
#' @returns A ggplot bar plot of the specified count type.
...
#' @param data_types list of 2 values (GEX, TCR), preferably alphanumeric
#' @param row_cluster if you would like to cluster the rows or not
#' @param col_cluster Whether or not to cluster the columns.
#' @param text_sizes A numeric vector of the inner text, border text, and title text sizes respectively.
#' @param matrix_annos A named list of annotations to display along the side if desired.
#' @param by_percent Whether the input matrix contains percentages or counts.
#'
#' @returns A ComplexHeatmap heatmap object showing barcode overlap.
plot_qc_heatmap <- function(input_matrix, data_types,
  ...
  ComplexHeatmap::Heatmap(matrix = input_matrix,
                          col = rev(pals::brewer.rdylbu(n = 7)), # like in pheatmap
                          name = legend_title,
...
#' @param subcluster Whether the data is subclustered.
...
plot_counts_cluster <- function(clusters, tissue_type = "", clrs_specific,
  ...
  # if you want to use the default ggplot2 colors
  if (rlang::is_missing(clrs_specific)) clrs_specific <- hue_pal()(n_distinct(clusters))

  data <- data.frame(table(clusters)) %>% rename(Count = Freq)

  if (!rlang::is_missing(add_zeroes)) {
...
#' @param details The optional subtitle.
#'
#' @returns A ggplot tile plot showing sample distribution across clusters.
...
#'
#' @description
#' Creates a summary table showing markers grouped by cell type from a filtered marker dataframe.
#'
#' @param filtered_markers_df A dataframe containing Cell_Type and Marker columns.
#'
#' @returns A tibble with Cell_Type and collapsed Markers columns.
...
#' @param label_largest_perc Whether to label the largest percentage.
#'
#' @returns A Seurat VlnPlot with cell count and percentage annotations.
vln_cell_info <- function(seurat_obj, feature, clrs_specific,
  ...
  y_cell_count <- y_exp_percent + 0.5
  y_max <- y_exp_percent + 1
  ...
  if (label_largest_perc) {
    ...
    y_cell_count <- y_exp_percent + 0.5
    y_largest_perc <- y_exp_percent + 1
    y_max <- y_exp_percent + 1.5
...
#'
#' @details
#' This assumes that colors_doublet has been defined and depends on other plots.
#'
#' @param seurat_obj The Seurat object.
#' @param tissue_type The tissue type of interest e.g. "Blood" or "Skin".
#' @param clrs_specific The specific color palette (should be named).
...
#' @param doublet_package The name of the doublet detection package used.
#' @param details The optional subtitle.
#'
#' @returns A patchwork plot combining UMAP and bar plots showing doublet identification results.
...
#' This function plots the annotated UMAPs alongside the TCR overlays.
#'
#' @details
#' lightgray is Seurat's default background color and cells_total has to be a list for the labels to work later.
#'
#' @param seurat_obj The Seurat object (must contain Has_TCR col)
#' @param tissue_type The tissue type of interest e.g. "Blood" or "Skin".
#' @param airr_type TCR
#' @param clrs_specific The specific color palette (should be named).
...
#'
#' @details
#' This function adds six new columns to the input dataframe: v_call_family, v_call_gene,
#' d_call_family, d_call_gene, j_call_family, and j_call_gene.
#'
...
#'
#' @details
#' For after add_family_info() has been run. This function ensures that gene
#' families and genes are ordered correctly (e.g., IGHV1, IGHV2, IGHV10 instead
#' of IGHV1, IGHV10, IGHV2).
#'
#' @param airr_db An AIRR-formatted data.frame.
#'
#' @returns A data.frame with six columns converted to properly ordered factors.
...
#'
#' @details
#' Assumes that `seurat_obj` contains `cell_id`.
#' Assumes that the TCR data includes alpha chains.
#' Collapsed light chains are alphabetized.
#'
#' @param seurat_obj The Seurat object containing GEX data.
#' @param airr_type Type of immune receptor data.
#' @param combined_airr AIRR formatted data frame with beta and alpha chains.
...
gex_add_airr <- function(seurat_obj, airr_type = "TCR",
  ...
  for (col in c("paired_alpha", "c_call", "locus")) {
  ...
  # integrate TCR data with the GEX Seurat object
...
#' Check AIRR and GEX data by cluster.
#'
#' @description
#' Creates a summary table showing percentages of AIRR data and main cell types by cluster.
#'
#' @details
...
#' @param annotations_col The column with cell type annotations.
#' @param airr_type Type of immune receptor data.
#' @param cell_types A character vector of cell types to include in the analysis.
#' @param paired Whether to include paired light/alpha chain information.
#'
#' @returns A data frame with percentages for the AIRR data and main cell type by cluster.
clusters_gex_airr <- function(seurat_obj, clusters_col, annotations_col,
                              airr_type = "TCR", cell_types, paired = TRUE) {
  airr_col <- paste0("Has_", airr_type)
  ...
  if (paired) {
    if (airr_type == "TCR") paired_col <- "paired_alpha"
...
#' @param output_summary Whether to print a summary of the matching results.
#'
#' @returns A character vector of original cluster identities.
match_sub_orig <- function(seurat_obj, seurat_obj_sub, cell_types,
  ...
  } else { # pull just the clusters that were used for their GEX labels
    cluster_info <- dplyr::filter(cluster_info,
...
#' This function replaces T cell type labels with annotated subpopulations based on cell ID.
#'
#' @param seurat_obj The Seurat object for the full tissue.
...
recombine_subclusters <- function(seurat_obj, seurat_obj_t,
                                  annotations_col = "annotated_clusters",
                                  annotations_col_t = "annotated_subclusters",
                                  annotations_col_new = "annotated_clusters_expanded") {
  # create a new metadata column with the T cell info
  new_col <-
    left_join(seurat_obj[[]],
              seurat_obj_t[[]] %>% select(Cell_ID_Unique, all_of(annotations_col_t)),
              by = "Cell_ID_Unique")
...
#' @param facet_by The variable to facet by.
#' @param points_by The variable for the points on top of the boxplot.
#' @param paired Whether or not the data has paired control and disease samples.
...
#' @param include_signif Whether to include significance test results.
#' @param signif_stars Whether to use star notation for significance levels.
#' @param signif_test The statistical test to use for significance.
#' @param signif_caption Whether to include a caption explaining significance notation.
#' @param details The optional subtitle.
#'
#' @returns A ggplot faceted plot showing subset frequencies with optional significance markers.
...
#'
#' @description
#' Performs differential expression analysis within each cell type using Seurat's FindMarkers function.
#'
#' @details
#' Custom fork of Seurat is available at https://github.com/jordansquair/Seurat
#'
#' @param sc A Seurat object with 'cell_type' and 'label' metadata columns.
#' @param de_test The test to use for differential expression analysis (e.g., 'wilcox', 'bimod').
#' @param params A list of additional parameters for the DE test.
#'
#' @returns A list of data frames containing differential expression results for each cell type.
...
#'
#' @description
#' Performs pseudobulk differential expression analysis using various DE testing methods.
#'
#' @details
#' The reference comparison should be first in the label. Metadata must contain a 'replicate' column.
#'
#' @param sc A Seurat object with 'cell_type', 'label', and 'replicate' metadata columns.
#' @param de_test The DE testing method: 'pseudobulk_limma', 'pseudobulk_DESeq2', or 'pseudobulk_edgeR'.
#' @param params A list of additional parameters for the chosen DE test method.
#'
#' @returns A list of data frames containing pseudobulk DE results for each cell type.
    ...
    DE = pseudobulks %>%
      purrr::map(function(x) {
        tryCatch({
          ...
          targets = data.frame(group_sample = colnames(x)) %>%
            mutate(group = gsub(".*\\:", "", group_sample))
          ## optionally, carry over factor levels from the entire dataset
          if (is.factor(meta$label)) {
            targets$group %<>% factor(levels = levels(meta$label))
          }
          ...
          if (!"test" %in% names(params)) params$test = "LRT"
          if (!"fitType" %in% names(params)) params$fitType = "parametric"
          if (!"sfType" %in% names(params)) params$sfType = "poscounts"
          if (!"independentFiltering" %in% names(params)) params$independentFiltering = FALSE
          if (!"betaPrior" %in% names(params)) params$betaPrior = FALSE
    ...
    DE = pseudobulks %>%
      purrr::map(function(x) {
        tryCatch({
          ...
          # set defaults on params
          if (!"test" %in% names(params)) params$test = "LRT"
          if (!"method" %in% names(params)) params$method = "TMM"
          if (!"trend.method" %in% names(params)) params$trend.method = "locfit"
          if (!"robust" %in% names(params)) params$robust = FALSE
          if (!"tagwise" %in% names(params)) params$tagwise = TRUE
          if (!"prior" %in% names(params)) params$prior = NULL
...
#'
#' @description
#' Wrapper function that runs differential expression analysis using either Seurat or pseudobulk methods.
#'
#' @param sc A Seurat object with 'cell_type', 'label', and optionally 'replicate' metadata columns.
#' @param de_test The DE testing method with optional parameters (e.g., 'pseudobulk_limma,mode?trend').
#' @param params A list of additional parameters for the chosen DE test method.
#' @param ... Additional arguments passed to the DE testing functions.
#'
#' @returns A data frame containing differential expression results for all cell types.
...
#' the legends
#'
#' @param DESeq2_out A list of data frames containing DE results from DESeq2 analysis.
#' @param gene_list A character vector of gene names to display in the heatmap.
#' @param FDR_cutoff The adjusted p-value cutoff for significance.
#' @param anno_top An optional data frame for top annotations (e.g., cell type).
#' @param clrs_anno_top A named list of colors for the top annotations.
#' @param clr_na The color to use for NA values in the heatmap.
#' @param clrs_plot A color palette function for the main heatmap values.
#' @param nclrs The number of colors to generate for the heatmap.
#' @param zero_white Whether to set zero fold-change values to white color.
#' @param split_row A data frame with "Type" and "Gene" columns for row grouping.
...
#' @param cell_height The height of each heatmap cell in points.
#' @param row_size The font size of row names on the right side.
#' @param col_size The font size of column names on the bottom.
#' @param number_size The font size of the significance asterisks.
#' @param flip_rows Whether to flip rows (rotate heatmap 90 degrees).
#' @param draw Whether to draw the heatmap (set to FALSE when combining heatmaps).
#'
#' @returns A ComplexHeatmap heatmap object.
heatmap_DESeq2 <- function(DESeq2_out, gene_list, FDR_cutoff = 0.05,
                           anno_top, clrs_anno_top, clr_na = "gray70",
                           clrs_plot =
