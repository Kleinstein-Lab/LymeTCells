...
Make sure to change the directories as needed based upon your file structure. If you are downloading the objects directly from GEO, then append "GSE296972_" to the beginning of their names (only for the primary blood and skin objects). Objects that are not on GEO were generated in one of the notebooks included in this repository.
...
table_clinical <-
  meta_clinical %>%
    select(-c("Year of Enrollment", "Antibiotic Name", "Dosage/duration")) %>%
    rename(Subject = "Study ID", Sex = Gender, EM = "Number of EM lesions",
           "Lesion Site(s)" = "Location of EM", Size = "Size (diameter in cm)",
           "Paired Control" = "Paired uninvolved skin sample") %>%
    mutate(Subject = as.character(Subject), Size = as.character(Size),
           EM = case_when(EM == 1 ~ "Single", EM > 1 ~ "Multiple"),
           Dataset = as.character(as.integer(Dataset) - 1)) %>%
    relocate(Dataset, .before = Subject) %>%
    relocate(Size, .before = "Lesion Site(s)")
```
