...
path_base <- file.path("~", "git", "Research", "LymeTCells")
...
packages <- c(# Immcantation packages
              "alakazam", "scoper", 
              # needed for manuscript figures
              "APackOfTheClones", "CellChat", "clusterProfiler", "ggpubr",
              "msigdbr", "SeuratExtend", "xlsx",
              # only for functions
              "circlize", "DESeq2", "DT", "ggsignif", "kableExtra", "knitr",
              "magrittr", "peakRAM", "rlang",
              # splicing
              "tximport",
              # plotting
              "ggrepel", "ggtext", "ggVennDiagram", "hues", "pals", "patchwork",
              # other
              "clustree", "here", "Matrix", "plyr", "reshape", "rjson",
              "scales", "Seurat", "tidyverse")
...
```{r colors}
# we had a dataset 1 that was not included in this manuscript
...
colors_sample_type_skin <- c("Control" = "#A6CEE3", "Control_EM" = "#00E321", 
                             "EM" = "#FF7F00") # pale blue, green, orange
...
```{r ggplot-themes}
# yes I know that I could have picked a name that wouldn't seem misspelled
theme_bw <- theme_bw() +
              theme(panel.grid.major.x = element_blank(),
                    panel.grid.major.y =
                      element_line(color = "grey75", linewidth = 0.2))
...
You shouldn't just pull out genes that begin with "IG" or "TR"
...
# there are some genes that start with "IG" or "TR" that are not immune-related,
# so you shouldn't just remove genes based upon that
biotypes_excl <-
...
If you want to download an Ensembl database yourself, you can use this code:

```{r}
#| eval = FALSE

# credit to Edward Lee and Hailong Meng in the Kleinstein lab
# make sure that you have biomaRt and tidyverse loaded

# define your species and genome of interest (use listDatasets below for other choices)
species <- "human" # or "mouse"
genome <- "hsapiens" # or "mmusculus"
...
# or pick an older version

# search Ensembl's datasets
ensembl <- useEnsembl(biomart = "ensembl")
dataset <- searchDatasets(mart = ensembl, pattern = genome)$dataset

# use listEnsembl() for biomart options (we always use genes)
# you could set mirror = "" but you'd have to remove the version parameter
...
attributes <- dplyr::filter(attributes,
                            name %in% c("ensembl_gene_id", "external_gene_name",
                                        "gene_biotype", "hgnc_symbol",
                                        "description") & page == "feature_page")
...
features_meta <- getBM(attributes = c("ensembl_gene_id", "external_gene_name",
                                      "gene_biotype", "hgnc_symbol",
                                      "description"),
                       filters = "biotype", values = tr_ig_biotypes,
                       mart = ensembl)

# save the file (change as desired)
file_name <- paste0("QC_features_meta_", species, "_v", current_version, ".rds")
write_rds(features_meta, file = file.path(path_resources, file_name))
```
